//Some includes: one for your class, and a handful of runtime support headers.
...
const char *str = shared_module_mymodule_myclass_get_question(self_in);
...
//Finally there's code that defines the class locals. This binds the method names to the interface functions defines above.
